/* sdlsound.c: SDL sound I/O
   ...
   $Id: sdlsound.c 804 2016-06-01 10:46:07Z fredm $
...
{
  int i = 0;
  libspectrum_signed_word volume_scaled_data[len];

  for( i=0; i<len; i++ ) {
    volume_scaled_data[i] = data[i] >> settings_current.volume;
  }

  /* Convert to bytes */
  libspectrum_signed_byte* bytes = (libspectrum_signed_byte*)volume_scaled_data;
  len <<= 1;
