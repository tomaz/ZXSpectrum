...
sound_lowlevel_init(const char *dev, int *freqptr, int *stereoptr) {
	...
	*freqptr = deviceFormat.mSampleRate;
	...
	if ((error = sfifo_init(&sound_fifo, NUM_FRAMES * deviceFormat.mBytesPerFrame * deviceFormat.mChannelsPerFrame * sound_framesize + 1))) {
		ui_error(UI_ERROR_ERROR, "Problem initialising sound fifo: %s", strerror(error));
		return 1;
	}
...
sound_lowlevel_frame(libspectrum_signed_word *data, int len) {
	...
	if (i < 0) {
		ui_error(UI_ERROR_ERROR, "Couldn't write sound fifo: %s", strerror(i));
	}
...
OSStatus coreaudiowrite(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, AudioBufferList *ioData) {
	int f;
