...
public class IconsStyleKit : NSObject {
    ...
    @objc dynamic public class func drawIconCheck(frame targetFrame: CGRect = CGRect(x: 0, y: 0, width: 22, height: 22), resizing: ResizingBehavior = .aspectFit) {
        //// General Declarations
        let context = UIGraphicsGetCurrentContext()!
        ...
        //// Bezier Drawing
        let bezierPath = UIBezierPath()
        bezierPath.move(to: CGPoint(x: 4.5, y: 9.85))
        bezierPath.addLine(to: CGPoint(x: 9.09, y: 17.5))
    ...
    @objc dynamic public class var imageOfIconCheck: UIImage {
        ...
        UIGraphicsBeginImageContextWithOptions(CGSize(width: 22, height: 22), false, 0)
            IconsStyleKit.drawIconCheck()

        Cache.imageOfIconCheck = UIGraphicsGetImageFromCurrentImageContext()!.withRenderingMode(.alwaysTemplate)
        UIGraphicsEndImageContext()
    ...
    @objc @IBOutlet dynamic var iconCheckTargets: [AnyObject]! {
        ...
        set {
            ...
            for target: AnyObject in newValue {
                let _ = target.perform(NSSelectorFromString("setImage:"), with: IconsStyleKit.imageOfIconCheck)
            }
        }
    }
